...
class ItemGateModule(nn.Module):
    def __init__(self, d_model: int, num_domains: int, num_item_tokens: int):
        ...
        self.cross = nn.ModuleList([nn.Linear(d_model * 2, d_model) for _ in range(num_domains)])
        self.gate = nn.ModuleList([nn.Linear(d_model * 2, 1) for _ in range(num_domains)])
    ...
    def forward(self, item_tokens, domain_tokens, domain_idx):
        ...
        attn_logits = self.attn(item_tokens)
        attn_w = attn_logits.softmax(dim=1)
        item_pooled = (attn_w * item_tokens).sum(dim=1, keepdim=True)
        item_exp = item_pooled.expand(-1, N, -1)
        concat = torch.cat([item_exp, domain_tokens], dim=-1)
        g = self.gate[domain_idx](concat).sigmoid()
        c = self.cross[domain_idx](concat)
        return domain_tokens + g * c
